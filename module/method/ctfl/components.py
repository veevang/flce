...
def extract_rules(prev_layer, skip_connect_layer, layer, pos_shift=0):
    ...
    tmp = 0
    rule_list = []
    Wb = (layer.W > 0.5).type(torch.int).detach().cpu().numpy()  # do*di
    ...
    for ri, row in enumerate(Wb):
        if layer.node_activation_cnt[ri + pos_shift] == 0 or layer.node_activation_cnt[
            ri + pos_shift] == layer.forward_tot:
            dim2id[ri + pos_shift] = -1  # all/no action, meaningless
            continue
        rule = {}
        bound = {}
        ...
        for i, w in enumerate(row):  # iterate for one rule
